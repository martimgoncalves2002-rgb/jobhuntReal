...
def _has_word(text, kw):
    """True if kw appears as a whole word/phrase (not inside another word).
    So 'graduate' matches 'graduate analyst' but NOT 'undergraduate'."""
    kw = kw.lower().strip()
    # \b doesn't play well with leading/trailing hyphens, so guard manually.
...
def score_job(job, fcfg):
    ...
    # LOCATION: only trust the job's location field, NOT the description —
    # otherwise a Boston role whose blurb mentions "London" leaks through.
