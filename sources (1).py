...
# ------------------------------------------------------------------ Reed
def fetch_reed(cfg):
    """Reed.co.uk Jobseeker API. Auth = API key as basic-auth username,
    empty password. Key comes from the REED_API_KEY secret via config."""
    api_key = (cfg or {}).get("api_key") or ""
    if not api_key:
        return []
    where = cfg.get("where", "London")
    distance = int(cfg.get("distance_from_location", 10))
    per_query = int(cfg.get("results_per_query", 100))
    out = []
    for query in cfg.get("queries", []):
        ...
        try:
            r = requests.get(url, params=params, headers=HEADERS,
                             auth=(api_key, ""), timeout=TIMEOUT)
            r.raise_for_status()
            results = r.json().get("results", [])
        except Exception as e:
            print(f"  [reed] '{query}' failed: {e}")
            continue
        for j in results:
            jid = str(j.get("jobId", "")) or _hash(j.get("jobUrl", ""))
            title = j.get("jobTitle", "") or ""
            desc = j.get("jobDescription", "") or ""
            # Reed dates are dd/mm/yyyy; keep as-is, it's only for display.
            out.append({
                "id": f"reed:{jid}",
                "title": _strip_html(title),
                "company": j.get("employerName", "") or "Unknown",
...
def fetch_all(config):
    ...
    jobs += adz
    reed = fetch_reed(config.get("reed") or {})
    print(f"  reed: {len(reed)}")
    jobs += reed
